...
const roles = mode === 'deployer' ? ['db_ddladmin', 'db_datareader', 'db_datawriter'] : [];
...
const grants = mode === 'backend'
  ? `GRANT SELECT, INSERT, UPDATE, DELETE, EXECUTE TO [${escaped}];`
  : mode === 'worker'
    ? `GRANT SELECT, INSERT, UPDATE, DELETE TO [${escaped}];`
    : `GRANT ALTER ANY USER TO [${escaped}]; GRANT SELECT, INSERT, UPDATE, DELETE, EXECUTE TO [${escaped}] WITH GRANT OPTION;`;
const query = `
