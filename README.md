...
Outbound status emails (queued, submitted, needs action) are also mirrored into the user's ApplyPilot inbox (`dbo.InboundMessages`) so the Email Inbox tab stays populated even when recruiters never reply. Simple Apply and the browser worker put the signed-in user email on employer forms (not a Postmark inbound alias), so verification codes and recruiter mail go to the user's real inbox.

Inbound mail via Postmark remains available for optional mailbox routing. Until the custom MX domain is verified, plus-addressed aliases under `POSTMARK_INBOUND_ADDRESS` can still receive inbound webhooks. After Postmark verifies `applypilotmail.accesscam.org`, set `mailboxDomain` in `infra/dev.bicepparam` for the shorter `alias@applypilotmail.accesscam.org` form. The webhook is an Azure Function with `function` authorization; never commit or publish its function key.
