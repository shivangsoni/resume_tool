...
The Function App uses Microsoft Entra managed identity. A SQL administrator must run `bootstrap/001_function_identity.sql` after replacing the Function App name. GitHub Actions uses the separately scoped principal in the idempotent `bootstrap/002_github_identity.sql`; run that bootstrap once before enabling automated migrations and again after bootstrap permission changes. It grants `ALTER ANY USER` plus grant-option authority for the application data permissions, so CI can recreate runtime identities without receiving `db_owner`. No SQL password is stored in application settings.
